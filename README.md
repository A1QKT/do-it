...
| `GET` | `/v1/media-seed/latest` | **`routes`**: **`route`**, **`node`** (true = junction like Ngã 4 …, false = corridor segment), **`score`**, **`reason`**. Images → **`route` = Ngã 4 Phú Nhuận**, **`node` = true**. **`MEDIA_SEED_IMAGE_ROUTE_NAME`**, **`full=true`**, **`MEDIA_SEED_EXPORT_PATH`**. |
