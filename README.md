...
| `GET` | `/v1/media-seed/latest` | **`routes`** only: **`route`** (short label from OpenRouter `analysis` or heuristics), **`score`**, **`reason`**. Query **`full=true`** for raw export. Env **`MEDIA_SEED_EXPORT_PATH`**. |
